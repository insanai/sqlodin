SQLodin's first binary release provides a replicated SQLite service, an interactive SQL client, and a bundled local SQLite shell.
...
Release tests run on Linux only. The supported deployment scope is a fixed three-voter cluster with matching builds and deterministic, bounded SQL. Any voter can accept writes; a majority is required for progress. The repository retains the formal-model checks, fault-test evidence, and measured performance results. These are evidence within stated assumptions, not a guarantee against every failure.
