...
Init == /\ offered = {} /\ votes = {} /\ chosen = {}
        /\ seen = [n \in Nodes |-> IF Producer = 0 THEN Target ELSE 0]
        /\ applied = [n \in Nodes |-> 0]
        /\ floor = [n \in Nodes |-> 0]
        /\ held = [n \in Nodes |-> [c \in Cells |-> 0]]
        /\ lost = FALSE
\* With all owners producing, seen does not guard any enabled action: Suggest
\* ignores it and Recover has no failed owner. Normalize that irrelevant field
\* to Target instead of enumerating equivalent observation orders. Skewed and
\* failed-owner configurations retain independent per-voter observations.
