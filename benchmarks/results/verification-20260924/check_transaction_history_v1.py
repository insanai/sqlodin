...
def operation(cluster, engine, node, identifier, key):
    ...
    op = dict(id=identifier, node=node, start=time.monotonic_ns(),
              kind='read' if key is None else 'transaction', key=key, status='pending')
                ...
                for batch in range(rounds):
                    ...
                    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
                        ...
                        for i in range(8):
                            ...
                            key = rng.randint(1, 4) if i % 2 == 0 else None
...
def main():
    ...
    report = dict(complete=False, format=5, policy=8, seed=args.seed, rounds=args.rounds,
