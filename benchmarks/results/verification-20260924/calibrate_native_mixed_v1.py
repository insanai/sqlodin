...
def workload(seed, count, reads, payload):
    rng = random.Random(seed)
    result = []
    for _ in range(count):
        key = rng.randrange(1024)
        write = rng.randrange(100) >= reads
        text = (f'UPDATE account SET balance=balance+1,payload=zeroblob({payload}) WHERE id={key}' if write
...
def measure(factory, case, count):
    barrier = threading.Barrier(case['clients'])
    raw = []
    def worker(index):
        db = factory(index)
        ops = workload(20260925+index, count, case['read_percent'], case['payload'])
        try:
            ...
            for write, text in ops:
                ...
                rows.append(dict(start=started, end=time.monotonic(), write=write))
            return rows
        finally:
            db.close()
    ...
    for write in (False, True):
        values = sorted(1000*(x['end']-x['start']) for x in raw if x['write'] == write)
        if values:
    ...
    return dict(elapsed=elapsed, completed=len(raw), per_second=len(raw)/elapsed, latency_ms=latency, raw=raw)
...
def main():
    ...
    lib = library(shared)
    report = dict(complete=False, scope='closed-loop calibration, one-statement transactions; not full R6 matrix',
    ...
    try:
        for clients in args.clients:
            for reads in args.reads:
                case = dict(clients=clients, read_percent=reads, payload=256)
                ...
                with tempfile.TemporaryDirectory(dir=root) as work:
                    ...
                    setup = ['CREATE TABLE seed(id INTEGER PRIMARY KEY)',
                        ...
                        'INSERT INTO account SELECT a.id*32+b.id,0,zeroblob(256) FROM seed a,seed b']
                    ...
                    try:
                        ...
                        assert reference.scalar('SELECT sum(balance) FROM account') == sum(x['write'] for x in case['sqlite']['raw'])
                    finally:
                        reference.close()
                    ...
                    try:
                        ...
                        for node in (1, 2, 3):
                            with cluster.connect((node,)) as db:
                                assert db.query('SELECT sum(balance) FROM account').scalar() == sum(x['write'] for x in case['sqlodin']['raw'])
