...
def main():
    ...
    report = dict(schema_version=1, complete=False, started_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                  ...
                  workload=dict(operations=args.operations,warmup=args.warmup,concurrency=args.concurrency,repeats=args.repeats,
                                read_percent=70,write_percent=30), realworld=[],sequential_writes=[],
    ...
    if args.resume_from:
        ...
        source=args.resume_from.with_name('linux18-native-initial-runner.py')
        ...
        report['resumed_from']=dict(path=str(args.resume_from),sha256=hashlib.sha256(args.resume_from.read_bytes()).hexdigest(),
                                    ...
                                    reason='Sequential adapter now retries idempotent operations; retain completed mixed samples')
        ...
        for row in earlier['realworld']:
            ...
            report['realworld'].append(dict(row,sample_run_directory=earlier['run_directory']))
    try:
        for repeat in range(args.repeats):
            seed = 20260923 + repeat*100
            systems = [s for s in args.systems if s != 'cowsql']; random.Random(seed).shuffle(systems)
            for system in systems:
                if any(r['system']==system and r['repeat']==repeat for r in report['realworld']): continue
                print('RUN mixed',repeat,system,flush=True)
                folder = work/f'{repeat}-{system}-mixed'
                result = native_run(folder,args,seed) if system=='sqlodin' else previous.network_run(system,folder,args,seed,previous.unused_base_port())
                result['repeat']=repeat; report['realworld'].append(result); save(args.output,report)
            ...
            for system in systems:
                ...
                if system=='sqlodin': result=native_kv(folder,args)
                elif system=='cowsql': result=previous.cowsql_run(folder,args,previous.unused_base_port())
                else: result=network_kv(system,folder,args)
                result['repeat']=repeat; report['sequential_writes'].append(result); save(args.output,report)
