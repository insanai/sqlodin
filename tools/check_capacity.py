...
def verify(cluster, rows, payloads, seed):
    ...
    for node in (1, 2, 3):
        with cluster.connect((node,), timeout=60) as db:
            ...
            counted = 0
            # Full logical row coverage, with indexed bounded work per query.
            # SQLite's optimized unqualified count can scan a large B-tree in
            # one VM opcode and monopolize the serialized service owner.
            for first in range(0, rows, VERIFY_RANGE_ROWS):
                counted += db.query('SELECT count(*) FROM capacity WHERE id>=? AND id<?',
                                    (first, min(rows, first+VERIFY_RANGE_ROWS))).scalar()
            assert counted == rows, (node, counted, rows)
            for _ in range(8):
...
def main():
    ...
    report = dict(complete=False, stages=[], seed=20260925,
                  ...
                  verification=dict(kind='complete primary-key range counts plus payload samples',
                                    rows_per_query=VERIFY_RANGE_ROWS),
                  samples=[])
        ...
        with retained_workspace(base, report, resume) as work:
            ...
            try:
                ...
                with cluster.connect(timeout=60) as db:
                    if resume:
                        rows = db.query('SELECT max(id) FROM capacity').scalar()+1
                        assert db.query('SELECT min(id) FROM capacity').scalar() == 0
                        assert db.query('SELECT max(id) FROM capacity').scalar() == rows-1
