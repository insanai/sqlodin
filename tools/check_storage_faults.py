...
def case(binary, library, openssl, root, database, mode):
    ...
    try:
        ...
        if mode == 'short-write':
            actual, requested = map(int, (root/'triggered.partial').read_text().split('/'))
            assert 0 < actual < requested
            result['partial_write'] = dict(actual=actual, requested=requested)
...
def main():
    ...
    p.add_argument('--mode', action='append', choices=('write-enospc', 'short-write', 'sync-eio'))
    ...
    try:
        with tempfile.TemporaryDirectory(dir=scratch) as directory:
            ...
            for database in ('consensus.db-wal', 'node.db-wal'):
                for mode in (args.mode or ('write-enospc', 'short-write', 'sync-eio')):
